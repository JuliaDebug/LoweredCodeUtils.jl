...
function bodymethod(mkw::Method)
    ...
    while true
        ...
        fakeargs = Any[nothing for i = 1:length((framecode.scope::Method).nargs)]
    ...
    if isexpr(stmt, :call) && (f = (stmt::Expr).args[1]; isa(f, QuoteNode))
        ...
        hasself = let src=src
            any(i -> is_self_call(stmt, src.slotnames, i), 2:length((stmt::Expr).args))
        end
        hasself || return m
        f = f.value
        mths = methods(f)
...
if ccall(:jl_generating_output, Cint, ()) == 1
    ...
    @assert precompile(Tuple{typeof(rename_framemethods!),Any,Frame,Dict{Symbol,MethodInfo},Vector{NamedTuple{(:linetop, :linebody, :callee, :caller),Tuple{Int64,Int64,Symbol,Union{Bool,Symbol}}}},Dict{Symbol,Union{Bool,Symbol}}})
